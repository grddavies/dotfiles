...
echoinfo "Configuring Kanata"

# BOOTSTRAP ROOT/SUDO
if [ "$(id -u)" -ne 0 ]; then
  echoinfo "This script requires sudo privileges. Requesting elevation..."
  if sudo "$0" "$@"; then
    exit 0
  else
    # sudo execution failed
    echoerror "Failed to execute script with sudo"
    exit 1
  fi
fi
...
if ! curl -fSL -o "$KANATA_BIN_PATH" "$KANATA_URL"; then
